    <div class="mdl-grid">
        <div class="mdl-color--white mdl-shadow--2dp mdl-cell mdl-cell--12-col mdl-grid">
            ...
            <form class="form-horizontal" role="form" method="POST" action="{{ url('/user/update') }}" ng-controller="GridFormController"
                  ng-init="formUrl='{{ url('/user/update') }}'; formData={{ $user->toJson() }}" onsubmit="return false;">
                        ...
                        <select class="mdl-select__input" id="user_role" name="user_role">
                          <option value="">Please Select User Role</option>
                            @foreach($roles_list as $role)
                                <option @if($user_role->getId() === $role->getId()) selected @endif value="{{$role->getId()}}">{{$role->getName()}}</option>
                            @endforeach
                        </select>
                      </div>
